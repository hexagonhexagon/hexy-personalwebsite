<?php
require_once 'includes/nonce_csp.php';
...
function getReasonPhrase(int $status_code) {
    global $reason_phrases;
    return $reason_phrases[$status_code] ?? null;
}
...
function getDescription(int $status_code) {
    ...
    if ($status_code === 404) {
        ...
        return "Something has gone wrong, and the website can't display whatever it is you're trying to do. Perhaps you should try something else?";
    }
    else {
        return "Everything is fine, but for some reason you're on the error page. I wonder why that is?";
    }
}
...
<main>
    <h1><?= $code ?></h1>
    <h2><?= getReasonPhrase($code) ?></h2>
    <p><?= getDescription($code) ?></p>
    <p><a href="/">Try returning to the main page?</a></p>
</main>
<?php makeFooter(); ?>